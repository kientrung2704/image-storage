      ...
      <a-tab-pane key="1" tab="Photos">
        <div class="shared">
          <div v-if="shared.files && shared.files.length === 0" class="shared-header">
            Hiện tại không có ai chia sẻ ảnh với bạn
          </div>
      ...
      <a-tab-pane key="2" tab="Albums" force-render>
        <div class="shared">
          <div v-if="shared.albums && shared.albums.length === 0" class="shared-header">
            Hiện tại không có ai chia sẻ album với bạn
          </div>
            ...
            <div class="album-item" v-for="(album, index) in shared.albums" :key="index">
              <a-dropdown class="more">
                ...
                <template #overlay>
                  <a-menu>
                    <a-menu-item>
                      <span @click="removeAlbum(album.id)">Rời khỏi album</span>
                    </a-menu-item>
                  </a-menu>
                </template>
...
export default {
  ...
  methods: {
    async getList() {
      const res = await this.$store.dispatch('share/tome')
      this.shared = res
    },
    shareUser() {
    ...
    async removeAlbum(id) {
      ...
      await this.$store.dispatch('album/exit', id)
      await this.getList()
